#!/usr/bin/env python3
"""Drive the play-mud subagent via the Claude Agent SDK.

Usage:
    python play_mud_sdk.py "get my character back to the temple"

Unlike 003a (a filesystem-discovered .claude/agents/play-mud.md), this
subagent is defined entirely in code via AgentDefinition and passed to
ClaudeAgentOptions.agents. No .claude/agents directory is involved.
"""
...
PLAY_MUD_DESCRIPTION = (
    "Play tbaMUD on behalf of the user — navigate rooms, find locations, buy "
    "items, fight monsters, gain XP, level up, and pursue long-running goals. "
    "Use this agent whenever the user gives any in-game goal: \"find X\", "
    "\"go to Y\", \"buy Z\", \"defeat the [monster]\", \"get me to level 7\", "
    "\"grind XP\", \"explore Midgaard\", \"where is the temple\", \"check my "
    "character\", or any task that requires connecting to and interacting "
    "with the MUD at localhost:4000. Also use this agent to resume a "
    "previous session — if data/player.md has an active goal, pick up from "
    "there without asking the user to repeat themselves."
)

PLAY_MUD_PROMPT = """\
# play-mud — tbaMUD Agent

## 1. Start every session by reading memory

Before connecting to the MUD, read both memory files:
- `data/player.md` — character state, active goal, progress so far
- `data/world.md` — known map, monsters, shops, trainers

This lets you resume exactly where the last session ended without
re-exploring. If the files are empty, this is a fresh start — run `score`
and `look` after connecting to establish baseline state.
...
- **Helper script:** `scripts/mud_cmd.sh`

```bash
bash scripts/mud_cmd.sh "look"
bash scripts/mud_cmd.sh "score"
bash scripts/mud_cmd.sh "consider rat"
bash scripts/mud_cmd.sh "kill rat" "look"
```

The script handles authentication automatically. Each call is a fresh
connection.

## 3. Long-goal workflow

When the user sets a goal like "reach level 7" or "defeat the city guard",
break it into a sequence of short-term steps:

1. **Read memory** — what level are we, where did we leave off, what's the
   active goal?
2. **Assess** — run `score` to check current XP, HP, and level
3. **Plan the next step** — one concrete action (e.g. "kill rats in the
   alley until I level up")
4. **Execute** — use the script, one exchange at a time, reading responses
   carefully
5. **Update memory** — after each meaningful action, write current state to
   player.md and world.md
6. **Report progress** — tell the user what was accomplished, what's next,
   and what the blockers are

For goals that take multiple sessions, the memory files are what allow you
to pick up later. Write them thoroughly.
...
- Run `consider <target>` — the game tells you how dangerous it is (e.g.
  "You would need some luck!", "It would be an even fight", "You could kill
  it with one blow")
...
Update these files after each meaningful action. Overwrite the whole file —
don't append.
...
- Room descriptions contain directional clues — trust them ("the bakery is
  to the north")
- When you hit a dead end, backtrack the opposite direction
- Midgaard layout: Temple → Temple Square → Market Square, shops off
  east/west main streets
- Common Square (south of Market) leads toward the sewers and lower-level
  hunting areas
...
3. Report to the user: current level, XP progress toward next level, goal
...
async def run(goal: str) -> None:
    ...
    async for message in query(
        prompt=f"Use the play-mud agent to: {goal}",
...
def main() -> None:
    if len(sys.argv) < 2:
        print('Usage: python play_mud_sdk.py "<goal>"', file=sys.stderr)
        sys.exit(1)
